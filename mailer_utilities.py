...
class MailerUtilities:
    ...
    def get_uids_for_file(self, filename, **kwargs):
        ...
        mails = self.mailbox.fetch(OR(criteria, criteria_encoded), **kwargs, bulk=False, headers_only=False)
    ...
    def get_by_uids(self, uids: list[str]):
        return self.mailbox.fetch(AND(uid=",".join(uids)), bulk=False) if len(uids) > 0 else ()

    def transfer_mail_to_mailbox_and_archive(self, uids: Union["all", str, list[str]], target_mailbox: MailBox,
                                             print_info=False):
        if uids == 'all':
            mails = self.mailbox.fetch(AND(all=True), bulk=False, mark_seen=False)
        elif type(uids) == str:
